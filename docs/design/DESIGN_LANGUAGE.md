...
- **Masthead（マストヘッド）** — 全ページ共通の最上段（layout 直下に1つ、全画面で不変）。`サイト識別（mono ワードマーク＋タグライン）＋ナビ（mono テキストラベル、絵文字不使用）` の識別段と、`スタンス（受動観測・判断しない）＋監視カバレッジ＋直近署名チェックポイント（`seq`＋`▸検証` 動線）` の来歴段から成る。**信頼を自称せず検証可能性を提示する**（USWDS banner の逆張り）。「稼働中」「検証済✓」など能動・自称の表示はしない（受動原則／レコード単位の検証バッジ禁止 ADR-000014）。来歴段の要素はデータが無い間は表示しない（嘘の値を出さない）。`--surface-1`＋下辺 `--hairline-2`。
