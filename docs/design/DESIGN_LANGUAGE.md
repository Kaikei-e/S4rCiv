...
```css
:root { --bp-sm: 30rem; /* 480px */ --bp-md: 48rem; /* 768px */ --bp-lg: 55rem; /* 880px */ }
```
...
- **フィルタ** — 狭幅ではトグルで開閉（既定で閉、トグルに現在の絞り込み件数を併記）。主動線（タイムライン）を即座に見せる。`<details>` は閉時に `content-visibility` で内容を隠す実装があり幅依存の常時展開を CSS で強制できないため、明示的な開閉状態（`pointer: fine` / 広幅では常時展開）で実装する。
