...
同梱する Web フォント（IBM Plex Sans JP / IBM Plex Sans / IBM Plex Mono — © IBM Corp.、Zen Old Mincho — © The Zen Old Mincho Project Authors）は **SIL Open Font License 1.1** で、AGPL-3.0 への同梱が許諾されている。表示高速化のため `unicode-range` でサブセット化してセルフホスト配信する（第三者リクエストなし）。OFL 全文は `web/src/lib/fonts/OFL-*.txt`、帰属表示は [/attribution](web/src/routes/attribution/+page.svelte)。再生成は `pnpm fonts:build`。
